...
function SkillsSection() {
  const skills = useSkillsStore(useShallow((s) => s.skills));
  ...
  const handleResetDefaults = async () => {
    if (!confirm('Restore the 6 built-in skills to their default templates? Custom skills are kept.')) return;
    await window.sable.skills.resetDefaults();
  };

  return (
    <div>
      <div className="flex items-center justify-between mb-1.5">
        <label className="text-2xs font-semibold tracking-wider uppercase text-fg-dim">
          Skills
        </label>
        <button
          onClick={() => void handleResetDefaults()}
          className="text-2xs text-fg-mute hover:text-fg"
        >
          Reset built-ins
        </button>
      </div>
      <p className="text-2xs text-fg-dim mb-2">
        Named prompt templates. The composer's <span className="font-mono">✨</span> picker shows them all.
      </p>
      <div className="space-y-1.5">
        {skills.map((skill) => (
          <SkillRow
        ...
        {adding ? (
          <SkillEditor
            skill={{ id: '', label: '', description: '', template: '' }}
            isNew
...
function SkillRow({
  ...
  if (editing) {
    return <SkillEditor skill={skill} onCancel={onCancel} onSaved={onSaved} />;
  }

  const handleRemove = async (e: React.MouseEvent) => {
    e.stopPropagation();
    await window.sable.skills.remove(skill.id);
  };

  return (
    <button
      onClick={onEdit}
      className="w-full flex items-center gap-2 px-2.5 py-2 bg-bg-3 border border-border-strong rounded-md text-left hover:border-fg-dim group"
    >
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          {skill.builtin && (
            <span className="text-2xs px-1 py-0.5 rounded bg-bg-4 text-fg-dim leading-none">
              built-in
            </span>
          )}
        </div>
        <div className="text-2xs text-fg-mute leading-snug truncate">
          {skill.description || skill.template.slice(0, 80)}
        </div>
      </div>
      {!skill.builtin && (
        <span
          ...
          title="Delete skill"
          className="opacity-0 group-hover:opacity-100 text-fg-dim hover:text-red-300 leading-none px-1 text-base"
        >
          ×
        </span>
      )}
...
function SkillEditor({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await window.sable.skills.save(next);
      onSaved();
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="px-2.5 py-2.5 bg-bg-3 border border-accent rounded-md space-y-2">
      <div className="flex gap-2">
        <input
          ...
          placeholder="Label"
          className="flex-1 px-2 py-1 bg-bg-2 border border-border-strong rounded text-sm text-fg outline-none focus:border-accent placeholder:text-fg-dim"
        />
        <div className="flex items-center px-2 py-1 bg-bg-2 border border-border-strong rounded text-sm font-mono text-fg-mute">
          ...
          <input
            ...
            className="bg-transparent text-fg-mute outline-none w-[100px] disabled:opacity-50"
          />
        </div>
      </div>
      <input
        ...
        className="w-full px-2 py-1 bg-bg-2 border border-border-strong rounded text-sm text-fg outline-none focus:border-accent placeholder:text-fg-dim"
      />
      <textarea
        value={template}
        ...
        rows={4}
        className="w-full px-2 py-1.5 bg-bg-2 border border-border-strong rounded text-sm text-fg outline-none focus:border-accent placeholder:text-fg-dim font-mono leading-snug resize-y"
      />
      <div className="flex justify-end gap-1.5">
        <button
          onClick={onCancel}
          disabled={saving}
          className="px-3 py-1 text-2xs text-fg-mute bg-bg-2 border border-border-strong rounded hover:text-fg disabled:opacity-50"
        >
          Cancel
        </button>
        <button
          ...
          className="px-3 py-1 text-2xs text-accent-fg bg-accent rounded hover:opacity-90 disabled:opacity-30 disabled:cursor-default"
        >
          {isNew ? 'Create' : 'Save'}
        </button>
      </div>
